...
export default function Index() {
  ...
  useEffect(() => {
    myChart = echarts.init(dnsRef.current)
    ...
    getData("keyperformancedata").then((v) => {
      v.map((item) => {
        ...
        (xyLab[labKey] != undefined && (xyLab[labKey] += item.dnsTime * 1)) || (xyLab[labKey] == undefined && (xyLab[labKey] = item.dnsTime * 1))
      })
  ...
  return (
    <div className='dns' ref={dnsRef}>Dns dns</div>
  )
}
